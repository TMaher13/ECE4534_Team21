...
void MQTTClientCallback(int32_t event, void *metaData, uint32_t metaDateLen, void *data, uint32_t dataLen)
{
    ...
    switch((MQTTClient_EventCB)event)
    {
        ...
        case MQTTClient_RECV_CB_EVENT:
        {
            ...
            if (strncmp(receivedMetaData->topic, "RoverStatus", receivedMetaData->topLen) == 0)
            {
                receiveData.messageType = TIMER70_MESSAGE;
                writeQueue(receive_handle, &receiveData);
            }

            else if (strncmp(receivedMetaData->topic, "connor", receivedMetaData->topLen) == 0)
            {
                receiveData.messageType = TIMER500_MESSAGE;
                ...
                for (i=1; i<ret; ++i) {
                    if (strncmp(data + parse_tok[i].start, "type", parse_tok[i].end - parse_tok[i].start) == 0) {
                        receiveData.type = (int_least8_t) strtol(data + parse_tok[i + 1].start, (char**) NULL, 10);
                    }
                    else if (strncmp(data + parse_tok[i].start, "angle", parse_tok[i].end - parse_tok[i].start) == 0) {
                        receiveData.angle = (int_least8_t) strtol(data + parse_tok[i + 1].start, (char**) NULL, 10);
                    }
                    else if (strncmp(data + parse_tok[i].start, "inFront", parse_tok[i].end - parse_tok[i].start) == 0) {
                        receiveData.inFront = (int_least8_t) strtol(data + parse_tok[i + 1].start, (char**) NULL, 10);
                    }
                }
            ...
            else if (strncmp(receivedMetaData->topic, "IPSStatus", receivedMetaData->topLen) == 0)
            {

                receiveData.angle = atoi(&data[12]);

                receiveData.messageType = IPSMESSAGE;
